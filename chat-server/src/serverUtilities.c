...
/*
   ...
   DESCRIPTION: Initializes shared memory and
   attaches Masterlist pointer to it.

   PARAMETERS:
	int shmVal : value for generating our key from ftok
	Masterlist** mList : Pointer to Masterlist pointer. Used
		             to attach pointer to shared memory.

   RETURNS: int	: -1 if failure, the shared memory ID otherwise
*/
